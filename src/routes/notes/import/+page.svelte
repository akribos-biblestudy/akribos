...
<main class="mx-auto w-full max-w-5xl px-4 py-7 sm:px-6 sm:py-10" data-testid="obsidian-import">
	...
	<div class="mt-7 grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]">
		...
		>
			...
			{#if form?.preview}
				<article class="p-5 sm:p-7" data-testid="import-preview">
					<div class="flex flex-wrap items-center gap-2 text-xs">
						...
						>
							{kindLabel(form.preview.kind)}
						</span>
						<span class="font-mono text-stone-400">{form.preview.sourceFilename}</span>
						<span
					...
					<h2 class="mt-4 font-serif text-2xl font-semibold">{form.preview.title}</h2>

					{#if form.preview.tags.length > 0 || form.preview.passages.length > 0}
						<div class="mt-3 flex flex-wrap gap-1.5">
							{#each form.preview.tags as tag (tag)}
								<span class="metadata-chip"><Icon name="tag" class="size-3" />{tag}</span>
							{/each}
							{#each form.preview.passages as passage (passage.reference + passage.resourceId)}
					...
					{#if form.preview.warnings.length > 0}
						<section
						...
						>
							...
							<ul class="mt-2 list-disc space-y-1 pl-5 text-xs">
								{#each form.preview.warnings as warning, index (`${warning}-${index}`)}
									<li>{warning}</li>
								{/each}
							</ul>
						</section>
					...
					>
						...
						<textarea class="hidden" name="source" aria-hidden="true">{form.source}</textarea>
